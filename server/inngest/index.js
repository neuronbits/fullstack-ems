...
export const inngest = new Inngest({ id: "neuronbit" });
...
// Cron: check attendance at 11:30 AM 06:00 UTC and email aabsent employees
const attendanceReminderCron = inngest.createFunction(
    { id: "attendance-reminder-cron", triggers: [{ cron: "0 0 6 * * *" }] },
    async ({ event, step }) => {
        // Step 1: Get today's date range
        const today = await step.run("get-today-date", () => {
            const startUTC = new Date(new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Karachi" }) + "T00:00:00 +05:30");
        ...
        const absentEmployees = activeEmployees
            .filter((emp) => !onLeaveIds.includes(emp._id) && !checkedInIds.includes(emp._id))
            .map((e) => e._id);
        ...
        if (absentEmployees.length > 0) {
            await step.run("send-reminder-emails", async () => {
                const emailPromises = absentEmployees.map((emp) => {
                    // send email
                    // Attendance Reminder
                    const attendanceRemindertemplate = `
