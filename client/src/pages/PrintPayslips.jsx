...
const PrintPayslips = () => {
    ...
    if (!payslip) return <p className='text-center py-12 text-light-400'>Payslip not found</p>
    ...
    return (
        <div className='max-w-2xl mx-auto p-8 bg-white animate-fade-in'>
            <div className='text-center border-b border-slate-200 pb-6 mb-8'>
                ...
                <p className='text-sm mt-1 text-slate-500'>{format(new Date(payslip.year, payslip.month - 1), 'MMMM yyyy')}</p>
            </div>
                ...
                <table className='w-full text-sm'>
                    ...
                    <tbody>
                        <tr className='border-t border-slate-100'>
                            ...
                            <td className='py-3 px-4 text-right font-medium text-slate-900'>${payslip.basicSalary.toLocaleString()}</td>
                        </tr>
                        <tr className='border-t border-slate-100'>
                            <td className='py-3 px-4 text-slate-700'>Allowances</td>
                            <td className='py-3 px-4 text-right font-medium text-slate-900'>${payslip.allowances.toLocaleString()}</td>
                        </tr>
                        <tr className='border-t border-slate-100'>
                            <td className='py-3 px-4 text-slate-700'>Deductions</td>
                            <td className='py-3 px-4 text-right font-medium text-slate-900'>${payslip.deductions.toLocaleString()}</td>
                        </tr>
                        <tr className='border-t border-slate-100 bg-slate-50'>
                            <td className='py-3 px-4 text-slate-700'>Net Salary</td>
                            <td className='py-3 px-4 text-right font-medium text-slate-900'>${payslip.netSalary.toLocaleString()}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div className='text-center'>
                <button onClick={() => window.print()} className='btn-primary print:hidden mb-4'>Print Payslip</button>
                <p className='text-xs text-slate-400'>This is a computer-generated payslip. No signature required.</p>
            </div>
        </div>
    )
}
