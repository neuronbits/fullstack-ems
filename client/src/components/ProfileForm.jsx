            ...
            <div className='space-y-5'>
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    <div>
                        ...
                        <input name="firstName" disabled value={`${initialData.firstName} ${initialData.lastName}`} className='bg-slate-50 text-slate-400 cursor-not-allowed' />
                    </div>
                    <div>
                        <label className='block text-sm font-medium text-slate-700 mb-2'>Email</label>
                        <input name="email" disabled value={initialData.email} className='bg-slate-50 text-slate-400 cursor-not-allowed' />
                    </div>
                    <div className='sm:col-span-2'>
                        <label className='block text-sm font-medium text-slate-700 mb-2'>Position</label>
                        <input name="position" disabled value={initialData.position} className='bg-slate-50 text-slate-400 cursor-not-allowed' />
                    </div>
                </div>
                <div>
                    <label className='block text-sm font-medium text-slate-700 mb-2'>Bio</label>
                    <textarea name="bio" disabled={initialData.isDeleted} defaultValue={initialData.bio || ""} placeholder="Write a brief bio..."
                ...
                {initialData.isDeleted ? (
                    <div className='pt-2'>
                        <div className='p-4 bg-rose-50 border border-rose-200 rounded-xl text-center'>
                            <p className='w-1.5 h-1.5 rounded-full bg-rose-500 mt-1.5 shrink-0'>Account Deactivated</p>
                            <p className='text-sm text-rose-500 mt-0.5'>You can no longer update your profile</p>
                        </div>
                    </div>
                ) : (
                    <div className='flex justify-end pt-2'>
                        <button type='submit' disabled={loading} className='btn btn-primary flex items-center gap-2 justify-center w-full sm:w-auto'>
